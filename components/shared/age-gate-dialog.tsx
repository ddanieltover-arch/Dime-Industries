...
/**
 * Full-page age gate (not a Radix portal).
 * Important: empty initial action state must NOT count as "resolved" — that
 * previously kept the gate closed and left shop/PDP as an empty black shell.
 */
export function AgeGateDialog({ initiallyOpen }: { initiallyOpen: boolean }) {
  ...
  const gateSucceeded = Boolean(state.success) && !pending;

  useEffect(() => {
    if (gateSucceeded) router.refresh();
  ...
  if (!initiallyOpen || gateSucceeded) return null;
