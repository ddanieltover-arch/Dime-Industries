  ...
  return (
    <article className="group flex h-full flex-col bg-[var(--color-surface)] shadow-[0_0_0_transparent] transition-[background-color,transform,box-shadow] duration-[var(--motion-base)] ease-[var(--ease-out)] hover:-translate-y-1 hover:bg-[var(--color-surface-raised)] hover:shadow-[var(--shadow-card)]">
      ...
      <div className="flex flex-1 flex-col border-t border-[var(--color-border)] p-4">
        ...
        <h3 className="mt-2 font-[var(--font-display)] text-[var(--scale-base)] uppercase tracking-[0.04em] text-[var(--color-ink)] transition-colors group-hover:text-[var(--color-resin)]">
        ...
        {!product.inStock ? (
          ...
          <Link
            href={href}
            className="btn-outline mt-3 w-full py-2.5 text-center text-[10px] tracking-[0.12em]"
          >
            Select options
          </Link>
        ) : (
