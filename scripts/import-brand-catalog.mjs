 ...
 *
 * After import, run: node scripts/apply-reference-prices.mjs
 * (Eaze CA + Rolling Releaf MA reference sheet).
 *
...
/** @type {Record<string, number>} Eaze-derived bands (refined by apply-reference-prices). */
const PRICE_BANDS = {
  signature: 4000,
  "live-reserve": 4300,
  balanced: 5800,
...
async function main() {
  ...
  for (const url of productUrls) {
    ...
    // Prefer in-page product art over og:image (often the brand logo).
    const pageImgs = [
      ...
      .filter(
        (u) =>
          /^https?:\/\//i.test(u) &&
          /wp-content\/uploads|i\d\.wp\.com/i.test(u) &&
          !/logo|favicon|cropped|font|\.svg|untitled-design|bar-3d/i.test(u) &&
          /royalpear|aio|tank|softgel|gumm|battery|preroll|\.png/i.test(u)
      );
    const productImage =
      pageImgs.find((u) => /royalpear/i.test(u)) ||
      pageImgs.find((u) => /w=1024|fit=1024|w=1200/i.test(u)) ||
      pageImgs[0] ||
      (ogImage && !/logo|cropped|s0\.wp\.com\/_si/i.test(ogImage) ? ogImage : null);

    const imgDir = path.join(IMG_ROOT, metaInfo.slug);
    let imageUrl = null;
    const galleryUrls = [];
    if (productImage) {
      const ext = extFromUrl(productImage);
      const dest = path.join(imgDir, `primary${ext}`);
      const ok = await downloadImage(productImage, dest);
      if (ok) {
...
// Run pnpm catalog:prices after import to apply Eaze/Rolling Releaf reference retail prices.
...
export const REFERENCE_PRICING = false as const;
...
`;
  ...
  console.log("Next: pnpm catalog:prices  (apply Eaze / Rolling Releaf retail sheet)");
}
